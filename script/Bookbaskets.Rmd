...
We did this work to find a way to arrange the book displays in order to increase sales at Flourish and Blotts bookstore.  The data set given us contains 90,000+ historical sales transactions and we want to answer the following questions:
...
This creates a sparse matrix of books by transactions.
...
- Then we sort overall book count to get top twenty most popular books. 
...
```
####Support plots
The first graph shows relative support books above 0.8%.
...
We need an association rule model to list strong rules. For example, a customer buys book 1 also buys book 2.  This is similar to recommending items to purchase on Amazon.com.
We also want to remove large transaction with more than 200 books.  Those larger transaction with more than 200 books are treated as outliers.  
...
There are 56 books in Oprah Book Club.   
We want to know how many of those same books also existed in the transaction data set. 
There are fifty-six book titles in Oprah Book Club intersect with titles in customers' transaction data set.
...
Use Oprah Book Club books on the left hand side rule of the Apriori algorithm.  
This would give us what other books we should display for a typical Oprah Book Club reader.
We did trial and errors and end up picking support 0.0003, confidence at 0.6, minimum length at 2, maximum length at 5.
...
oprahRule <- apriori(bookbaskets_use, parameter = list(support = 0.00003, confidence = 0.6, minlen = 2, maxlen = 5),
...
basically all those rules on the top right corner. Most of rule has greater than 0.001 relative support and 0.8 confidence.
...
We can also create a grouped matrix plot for eighty-four rules.  
The second plot limit to top ten rules.

```{r}
## Grouped Matrix Plot
plot(bookbasketrules_sorted, method="grouped") 
plot(bookbasketrules_sorted, method="grouped", control=list(k=10)) 
```
...
We used the same 0.001 support and 0.8 confidence threshold.
...
- Oraph Book Club has fifty-six books in our transaction data set.  From those fifty-six books we discovered rules that book club reader would like to read.  For example, "The Treasure Hunt" and "True Crime: The Novel".  We can use the list to appeal book club readers.  The resulting rules are listed above.
