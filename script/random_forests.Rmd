...
```{r}
setwd("C:/Users/lj015625/Desktop/DataMining Class/Applied_Data_Mining/data")
...
```{r}
rpart.plot(prescribers_10folds$finalModel, type=0, extra=101)
prescribers_rpart_pruned_10folds<-prune(prescribers_10folds$finalModel, cp=0.06)
...
plot(BC_rpart_caret, uniform=TRUE, main="breast cancer")
...
library(ipred)
set.seed(123)
mybag <- bagging(Class ~ ., data = BC_train_caret, nbagg = 10)
```

```{r}
actual <- BC_validate_caret$Class 
BC_predicted <- predict(mybag, newdata=BC_validate_caret, type="class") 
...
mybag <- train(Class ~ ., data = BC_train_caret, method = "treebag", metric="Accuracy", na.action=na.omit, trControl = ctrl)
```
```{r}
actual <- BC_validate_caret$Class 
BC_predicted <- predict(mybag, newdata=BC_validate_caret, type="class") 
...
svmbag <- train(Class ~ ., data = BC_train_caret, "bag", trControl = ctrl, na.action=na.omit, bagControl = bagctrl)
```
...
set.seed(123)
BC_adaboost <- boosting.cv(Class ~.,data=BC_train_caret, boos=TRUE, v=30) #.cv is adding cross validation
